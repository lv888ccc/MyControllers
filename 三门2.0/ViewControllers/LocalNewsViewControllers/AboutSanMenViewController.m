...
#import "AboutSanMenViewController.h"
...
@interface AboutSanMenViewController ()<MyTableViewDelegate>
{
    NSInteger pageIndex;
}
...
@implementation AboutSanMenViewController
...
- (void)viewDidLoad
{
    ...
    self.myTable.cellNameString = @"AboutSanMenCell";
...
- (void) readLocalDB
{
    NSArray *tmpArr = [FMDBManage getDataFromTable:[LocalNewsObject class] WithString:[NSString stringWithFormat:@"l_newsType='%@'",LOCALNEWSABOUTSANMENMETHOD]];
    if (tmpArr)
    {
...
{
    pageIndex = 1;
    [self.myRequest localNewsAboutSanMenWithPageNo:pageIndex];
}
...
{
    LocalNewsObject *tmpObject = [self.tableData objectAtIndex:indexPath.row];
    NewsDetailViewController *newsDetail = [[NewsDetailViewController alloc] init];
    newsDetail.newsId = tmpObject.l_id;
...
#pragma mark - 网络数据放回
//县情-三门概况
- (void) localNewsAboutSanMenCallBack:(id) objectData
{
