...
#import "UserPaiKeViewController.h"
...
#define GUIDTAG 10000

@interface UserPaiKeViewController ()<TMQuiltViewDataSource,TMQuiltViewDelegate,EGORefreshTableHeaderDelegate,UIScrollViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
{
    //下拉刷新
...
@implementation UserPaiKeViewController
...
- (void) initViewController
{
    ...
    self.quiltView = [[TMQuiltView alloc] initWithFrame:CGRectMake(0, 60, 320, viewHeight - 109)];
    ...
    if ([self.settingObject.s_camara isEqualToString:@"-1"] || self.settingObject == nil)
    {
        UIButton *guidBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [guidBtn setImage:[UIImage imageNamed:@"paike_mark"] forState:UIControlStateNormal];
        [guidBtn setImage:[UIImage imageNamed:@"paike_mark"] forState:UIControlStateHighlighted];
        guidBtn.alpha = 0;
...
- (void) camaraCome
{
    UIButton *guidBtn = (UIButton *)[SMApp.nav.view viewWithTag:GUIDTAG];
...
- (void) reloadData
{
    ...
    [self.myRequest paikeUserWithPageNo:pageIndex];
}
...
- (void) loadMoreViewAnimationWithArr:(NSMutableArray *) arr
{
    if ([arr count] >= PAGESIZE)
    {
        ...
        rect.origin.y = self.quiltView.contentSize.height - 35;
        rect.origin.x = 150;
...
- (void)guidTapped:(UIButton *)sender
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        //入库
        [FMDBManage updateTable:self.settingObject setString:@"s_camara='0'" WithString:@"1=1"];
    }];
}
...
#pragma mark -
#pragma mark - 网络数据放回
- (void) paikeUserCallBack:(id) objectData
{
    [self finishLoading];
